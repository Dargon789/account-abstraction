  ...
  describe("Stake Management", () => {
    ...
    describe("without stake", () => {
      ...
      it("should fail to stake without delay", async () => {
        await expect(
          entryPoint.callStatic.addStake(0, { value: ONE_ETH }),
        ).to.revertedWith("must specify unstake delay");
      });
      it("should fail to unlock", async () => {
        await expect(entryPoint.callStatic.unlockStake()).to.revertedWith(
          "not staked",
        );
      });
    });
...
>>>>>>> Stashed changes
  ...
  describe("flickering account validation", () => {
    it("should prevent leakage of basefee", async function () {
      ...
      try {
        await simulateValidation(userOpPacked, entryPoint.address, {
          gasLimit: 1e6,
        });
...
>>>>>>> Stashed changes
        ...
        await ethers.provider.send("evm_mine", []);
        await expect(
          simulateValidation(userOpPacked, entryPoint.address, {
            gasLimit: 1e6,
          }),
        ).to.revertedWith("Revert after first validation");
<<<<<<< Updated upstream
...
>>>>>>> Stashed changes
        ...
        expect(decodeRevertReason(e)).to.include(
    ...
    it("should limit revert reason length before emitting it", async () => {
      ...
      await simulateValidation(badOpPacked, entryPoint.address, {
        gasLimit: 3e5,
      });
...
>>>>>>> Stashed changes
      ...
      describe("should pay prefund and revert account if prefund is not enough", function () {
        ...
        async function createUserOpWithGas(
          vgl: number,
          pmVgl: number,
          cgl: number,
...
>>>>>>> Stashed changes
          ...
          minCallGas = await findUserOpWithMin(
            async (cgl: number) => createUserOpWithGas(5e5, 0, cgl),
            true,
            entryPoint,
            1,
            100000,
...
>>>>>>> Stashed changes
      ...
      it("should reject non-contract (address(1)) aggregator", async () => {
        // this is just sanity check that the compiler indeed reverts on a call to "validateSignatures()" to nonexistent contracts
        const address1 = hexZeroPad("0x1", 20);
        const aggAccount1 = await new TestAggregatedAccount__factory(
          ethersSigner,
        ).deploy(entryPoint.address, address1);
...
>>>>>>> Stashed changes
      ...
      describe("execution ordering", () => {
        ...
        context("create account", () => {
          ...
          before(async () => {
            const factory = await new TestAggregatedAccountFactory__factory(
              ethersSigner,
            ).deploy(entryPoint.address, aggregator.address);
          ...
          it("simulateValidation should return aggregator and its stake", async () => {
            ...
            const { aggregatorInfo } = await simulateValidation(
              userOp,
              entryPoint.address,
            );
...
>>>>>>> Stashed changes
    it("should return true for pure EntryPoint, IStakeManager and INonceManager interface IDs", async function () {
      ...
      const inheritedMethods = new Set(
        [...smInterface.fragments, ...nmInterface.fragments].map((f) => f.name),
      );
      const epPureInterfaceFunctions = [
        ...epInterface.fragments.filter(
          (it) => !inheritedMethods.has(it.name) && it.type === "function",
        ),
      ];
      ...
      expect(await entryPoint.supportsInterface(epPureInterfaceID)).to.equal(
        true,
      );
    });
    ...
    it("should return false for a wrong interface", async function () {
      ...
      const entryPointInterfaceID = getERC165InterfaceID([
        ...saInterface.fragments,
      ]);
      expect(
        await entryPoint.supportsInterface(entryPointInterfaceID),
      ).to.equal(false);
    });
  });
});
