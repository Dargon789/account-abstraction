...
describe("#ValidationData helpers", function () {
  ...
  it("#parseValidationData", async () => {
    expect(await helpers.parseValidationData(0)).to.eql({
      aggregator: AddressZero,
      validAfter: 0,
      validUntil: max48,
    });
    expect(await helpers.parseValidationData(1)).to.eql({
      aggregator: addr1,
      validAfter: 0,
      validUntil: max48,
    });
    expect(
      await helpers.parseValidationData(
        packValidationData({
          aggregator: AddressZero,
          validUntil: 0,
          validAfter: 10,
  ...
  it("#parseValidationData (typescript)", async () => {
    expect(await helpers.parseValidationData(0)).to.eql(parseValidationData(0));
    expect(
      await helpers.parseValidationData(
        packValidationData({ aggregator: addr, validUntil: 10, validAfter: 0 }),
      ),
    ).to.eql(
      parseValidationData(
        packValidationData({ aggregator: addr, validUntil: 10, validAfter: 0 }),
  ...
  it("#packValidationData with aggregator", async () => {
    expect(
      hexlify(
        await helpers.packValidationDataStruct({
          aggregator: addr,
          validUntil: 234,
          validAfter: 567,
        }),
