...
describe("OracleHelper", function () {
  function testOracleFiguredPriceOut(): void {
    it("should figure out the correct price", async function () {
      ...
      const tokensPerEtherCalculated = await testEnv.paymaster.weiToToken(
        parseEther("1"),
        cachedPrice,
      );
      assert.equal(
        cachedPrice.toString(),
        testEnv.expectedPrice.toString(),
        "price not right",
      );
      assert.equal(
        tokensPerEtherCalculated.toString(),
        testEnv.expectedTokensPerEtherCalculated.toString(),
        "tokens amount not right",
      );
    });
  }
  ...
  function getOracleConfig({
    ...
    return {
      ...
      tokenOracleReverse,
      tokenToNativeOracle,
      nativeOracle: tokenToNativeOracle
        ? AddressZero
        : testEnv.nativeAssetOracle.address,
  ...
  before(async function () {
    ...
    testEnv.tokenOracle = await new TestOracle2__factory(ethersSigner).deploy(
      1,
      0,
    );
    testEnv.nativeAssetOracle = await new TestOracle2__factory(
      ethersSigner,
    ).deploy(1, 0);
  ...
  describe("with one-hop direct price ETH per TOKEN", function () {
    before(async function () {
      ...
      testEnv.expectedPrice = BigNumber.from(res.answer)
        .mul(priceDenominator)
        .div(tokenOracleDecimalPower)
        .toString();

      testEnv.expectedTokensPerEtherCalculated = BigNumber.from(parseEther("1"))
        .mul(tokenOracleDecimalPower)
        .div(res.answer)
        .toString();

      const ethersSigner = ethers.provider.getSigner();
      testEnv.paymaster = await new TokenPaymaster__factory(
        ethersSigner,
      ).deploy(
        testEnv.token.address,
        entryPoint.address,
  ...
  describe("with one-hop reverse price TOKEN per ETH", function () {
    before(async function () {
      ...
      await testEnv.nativeAssetOracle.setPrice("0xfffffffffffffffffffff");
      const tokenOracleDecimalPower = BigNumber.from(10).pow(res.decimals);
      testEnv.expectedPrice = BigNumber.from(priceDenominator)
        .mul(tokenOracleDecimalPower)
        .div(res.answer)
        .toString();

      const expectedTokensPerEtherCalculated = BigNumber.from(parseEther("1"))
        .mul(res.answer)
        .div(tokenOracleDecimalPower)
        .toString();
      ...
      assert.equal(
        expectedTokensPerEtherCalculated.toString(),
        testEnv.expectedTokensPerEtherCalculated.toString(),
      );
  ...
  describe("with two-hops price USD-per-TOKEN and USD-per-ETH", function () {
    before(async function () {
      ...
      testEnv.expectedPrice = priceDenominator
        .mul(resToken.answer)
