...
function uniq(arr: any[]): any[] {
  ...
  return Object.values(
    arr.reduce((set, item) => ({ ...set, [item.name]: item }), {}),
  );
}
...
describe("TokenPaymaster", function () {
  ...
  before(async function () {
    ...
    tokenOracle = await new TestOracle2__factory(ethersSigner).deploy(
      initialPriceToken,
      8,
    );
    ...
    callData = await account.populateTransaction
      .execute(accountOwner.address, 0, "0x")
      .then((tx) => tx.data!);
  });
  ...
  it("Only owner should withdraw eth from paymaster to destination", async function () {
    ...
    assert.equal(
      balanceBefore.add(BigNumber.from(amount)).toString(),
    ...
    const paymasterDifferentSigner = TokenPaymaster__factory.connect(
      paymasterAddress,
      impersonatedSigner,
    );
    ...
    await expect(
      paymasterDifferentSigner.withdrawEth(paymasterOwner, amount),
    ).to.be.revertedWith("OwnableUnauthorizedAccount");

    // should revert if the transfer fails
    await expect(
      paymaster.withdrawEth(recipient, BigNumber.from(amount).mul(2)),
    ...
    assert.equal(
      recipientBalanceBefore.add(BigNumber.from(amount)).toString(),
  ...
  it("paymaster should reject if account does not have enough tokens or allowance", async () => {
    ...
    expect(
      ...
      /FailedOpWithRevert\(0,"AA33 reverted",ERC20InsufficientAllowance/,
    );
    ...
    expect(
      ...
      /FailedOpWithRevert\(0,"AA33 reverted",ERC20InsufficientBalance/,
    );
  ...
  it("should be able to sponsor the UserOp while charging correct amount of ERC-20 tokens", async () => {
    ...
    const actualTokenPriceWithMarkup =
      decodedLogs[3].args.actualTokenPriceWithMarkup;
    ...
    const expectedTokenPriceWithMarkup = priceDenominator
      .mul(initialPriceToken)
      .div(initialPriceEther) // expectedTokenPrice of 0.2 as BigNumber
      .mul(10)
      .div(15); // added 150% priceMarkup
    const expectedTokenCharge = actualGasCostPaymaster
      .add(addedPostOpCost)
      .mul(priceDenominator)
      .div(expectedTokenPriceWithMarkup);
    ...
    assert.equal(
      actualTokenChargeEvents.toString(),
      actualTokenCharge.toString(),
    );
    assert.equal(
      actualTokenChargeEvents.toString(),
      expectedTokenCharge.toString(),
    );
    assert.equal(
      actualTokenPriceWithMarkup.toString(),
      expectedTokenPriceWithMarkup.toString(),
    );
    assert.closeTo(
      postOpGasCost.div(tx.effectiveGasPrice).toNumber(),
      50000,
      20000,
    );
    await ethers.provider.send("evm_revert", [snapshot]);
  ...
  it("should update cached token price if the change is above configured percentage", async function () {
    ...
    op = signUserOp(op, accountOwner, entryPoint.address, chainId);
    const opPacked = packUserOp(op);
    const tx: ContractTransaction = await entryPoint.handleOps(
      [opPacked],
      beneficiaryAddress,
      { gasLimit: 1e7 },
    );
    ...
    const oldExpectedPrice = priceDenominator
      .mul(initialPriceToken)
      .div(initialPriceEther);
    ...
    const actualTokenPriceWithMarkup =
      decodedLogs[4].args.actualTokenPriceWithMarkup;
    assert.equal(
      actualTokenPriceWithMarkup.toString(),
      newExpectedPriceWithMarkup.toString(),
    );
    await expect(tx)
  ...
  it("should use token price supplied by the client if it is better than cached", async function () {
    ...
    const preChargeTokens = decodedLogs[0].args.value;
    const requiredGas = BigNumber.from(op.callGasLimit)
      .add(
        BigNumber.from(op.verificationGasLimit).add(
          BigNumber.from(op.paymasterVerificationGasLimit),
        ),
      )
      .add(BigNumber.from(op.paymasterPostOpGasLimit))
      .add(op.preVerificationGas)
      .add(40000 /*  REFUND_POSTOP_COST */);
    const requiredPrefund = requiredGas.mul(op.maxFeePerGas);
    const preChargeTokenPrice = requiredPrefund
      .mul(priceDenominator)
      .div(preChargeTokens);

    // TODO: div 1e10 to hide rounding errors. look into it - 1e10 is too much.
    assert.equal(
      preChargeTokenPrice.div(1e10).toString(),
      overrideTokenPrice.div(1e10).toString(),
    );
    await ethers.provider.send("evm_revert", [snapshot]);
  });
  ...
  it("should use cached token price if the one supplied by the client is worse", async function () {
    ...
    assert.equal(
      preChargeTokenPrice.toString(),
      currentCachedPrice.mul(10).div(15).toString(),
    );
    await ethers.provider.send("evm_revert", [snapshot]);
  });
  ...
  it("should charge the overdraft tokens if the pre-charge ended up lower than the final transaction cost", async function () {
    ...
    await token.transfer(
      account.address,
      await token.balanceOf(await ethersSigner.getAddress()),
    );
    await token.sudoApprove(
      account.address,
      paymaster.address,
    ...
    assert.equal(
      preChargeTokens.add(overdraftTokens).toString(),
      actualTokenCharge.toString(),
    );
  ...
  it("should revert in the first postOp run if the pre-charge ended up lower than the final transaction cost but the client has no tokens to cover the overdraft", async function () {
    ...
    const withdrawTokensCall = await token.populateTransaction
      .transfer(token.address, parseEther("0.009"))
      .then((tx) => tx.data!);
    const callData = await account.populateTransaction
      .execute(token.address, 0, withdrawTokensCall)
      .then((tx) => tx.data!);
    ...
    console.log(
      decodedLogs.map((e: any) => ({ ev: e.name, ...objdump(e.args!) })),
    );

    const postOpRevertReason = decodeRevertReason(
      decodedLogs[2].args.revertReason,
    );
    assert.include(
      postOpRevertReason,
      "PostOpReverted(ERC20InsufficientBalance",
    );
  ...
  it("should swap tokens for ether if it falls below configured value and deposit it", async function () {
    ...
    const deFactoExchangeRate =
      decodedLogs[4].args.amountOut.toString() /
      decodedLogs[4].args.amountIn.toString();
