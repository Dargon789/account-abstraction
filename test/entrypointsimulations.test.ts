...
describe("EntryPointSimulations", function () {
  ...
  before(async function () {
    ...
    epSimulation = await new EntryPointSimulations__factory(
      provider.getSigner(),
    ).deploy();
...
>>>>>>> Stashed changes
    ...
    it("deposit on simulation must be >= real entrypoint", async () => {
      costInRange(
        ...
        await entryPoint.estimateGas.depositTo(addr, { value: 1 }),
        "deposit with value",
      );
    });
    it("deposit without value on simulation must be >= real entrypoint", async () => {
      costInRange(
        ...
        await entryPoint.estimateGas.depositTo(addr, { value: 0 }),
    ...
    it("eth transfer on simulation must be >= real entrypoint", async () => {
      costInRange(
        ...
        await provider.estimateGas({ to: entryPoint.address, value: 1 }),
        "eth transfer with value",
      );
    });
    it("eth transfer (even without value) on simulation must be >= real entrypoint", async () => {
      costInRange(
        ...
        await provider.estimateGas({ to: entryPoint.address, value: 0 }),
        "eth transfer with value",
      );
    });
  });
...
*/
  ...
  describe("#simulateValidation", () => {
    ...
    it("should fail if validateUserOp fails", async () => {
      ...
      await expect(simulateValidation(op, entryPoint.address)).to.revertedWith(
    ...
    it("should fail creation for wrong sender", async () => {
      ...
      await expect(simulateValidation(op1, entryPoint.address)).to.revertedWith(
    ...
    it("should report failure on insufficient verificationGas (OOG) for creation", async () => {
      ...
      await expect(
        simulateValidation(op1, entryPoint.address, { gas: "0xF4240" }),
...
=======
  ...
  describe("over-validation test", () => {
    ...
    before(async () => {
      owner = createAccountOwner();
      paymaster = await new TestPaymasterWithPostOp__factory(
        ethersSigner,
      ...
      sender = account.address;
      await fund(account);
      pmVgl = await findSimulationUserOpWithMin(
        async (n) => userOpWithGas(1e6, n),
        entryPoint,
        1,
        500000,
      );
      vgl = await findSimulationUserOpWithMin(
        async (n) => userOpWithGas(n, pmVgl),
        entryPoint,
        3000,
        500000,
      );
      ...
      await simulateValidation(packUserOp(userOp), entryPoint.address).catch(
        (e) => {
          throw new Error(decodeRevertReason(e)!);
        },
      );
    ...
    describe("compare to execution", () => {
      ...
      const diff = 2000;
      before(async () => {
        execPmVgl = await findUserOpWithMin(
          async (n) => userOpWithGas(1e6, n),
          false,
          entryPoint,
          1,
        ...
        execVgl = await findUserOpWithMin(
          async (n) => userOpWithGas(n, execPmVgl),
          false,
          entryPoint,
          1,
          500000,
      ...
      it("account verification simulation cost should be higher than execution", function () {
        console.log(
          "simulation account validation",
          vgl,
          "above exec:",
          vgl - execVgl,
        );
        expect(vgl).to.be.within(
          execVgl + 1,
          execVgl + diff,
          `expected simulation verificationGas to be 1..${diff} above actual, but was ${vgl - execVgl}`,
        );
      });
      it("paymaster verification simulation cost should be higher than execution", function () {
        console.log(
          "simulation paymaster validation",
          pmVgl,
          "above exec:",
          pmVgl - execPmVgl,
        );
        expect(pmVgl).to.be.within(
          execPmVgl + 1,
          execPmVgl + diff,
          `expected simulation verificationGas to be 1..${diff} above actual, but was ${pmVgl - execPmVgl}`,
        );
      });
    });
    it("should revert with AA2x if verificationGasLimit is low", async function () {
      expect(
        await simulateValidation(
          packUserOp(await userOpWithGas(vgl - 1, pmVgl)),
          entryPoint.address,
        ).catch(decodeRevertReason),
      ).to.match(/AA26/);
    });
    it("should revert with AA3x if paymasterVerificationGasLimit is low", async function () {
      expect(
        await simulateValidation(
          packUserOp(await userOpWithGas(vgl, pmVgl - 1)),
          entryPoint.address,
        ).catch(decodeRevertReason),
      ).to.match(/AA36/);
    });
...
>>>>>>> Stashed changes
  ...
  describe("#simulateHandleOp", () => {
    it("should simulate creation", async () => {
      const accountOwner1 = createAccountOwner();
      const factory = await new SimpleAccountFactory__factory(
        ethersSigner,
      ).deploy(entryPoint.address);
      ...
      const ret = await simulateHandleOp(
        userOp,
        counter.address,
