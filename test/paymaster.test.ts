...
describe("EntryPoint with paymaster", function () {
  ...
  describe("#TokenPaymaster", () => {
    ...
    it("paymaster should revert on wrong entryPoint type", async () => {
      ...
      await expect(
        new LegacyTokenPaymaster__factory(ethersSigner).deploy(
          factory.address,
          "ttt",
          notEntryPoint.address,
        ),
    ...
    it("owner should have allowance to withdraw funds", async () => {
      expect(await paymaster.allowance(pmAddr, ownerAddr)).to.equal(
    ...
    it("should allow only NEW owner to move funds after transferOwnership", async () => {
      ...
      expect(await paymaster.allowance(pmAddr, otherAddr)).to.equal(
        ethers.constants.MaxUint256,
      );
  ...
  describe("using TokenPaymaster (account pays in paymaster tokens)", () => {
    ...
    describe("create account", () => {
      ...
      it("should succeed to create account with tokens", async () => {
        ...
        await simulateValidation(createOp, entryPoint.address, {
          gasLimit: 5e6,
        });
      ...
      it("batched request should each pay for its share", async function () {
        ...
        const execFromSingleton = account.interface.encodeFunctionData(
          "execute",
          [testCounter.address, 0, justEmit],
        );
        ...
        for (let i = 0; i < 4; i++) {
          const { proxy: aAccount } = await createAccount(
            ethersSigner,
            await accountOwner.getAddress(),
            entryPoint.address,
          );
        ...
        const totalPaid =
          (await paymaster
            .balanceOf(paymaster.address)
            .then((b) => b.toNumber())) - pmBalanceBefore;
      ...
      describe("grief attempt", () => {
        ...
        before(async function () {
          ...
          expect(
            await paymaster.allowance(account2.address, account.address),
        ...
        it("griefing attempt in postOp should cause the execution part of UserOp to revert", async () => {
          ...
          const withdrawTokens = paymaster.interface.encodeFunctionData(
            "transferFrom",
            [account2.address, account.address, withdrawAmount],
          );
          const execFromEntryPoint = account.interface.encodeFunctionData(
            "execute",
            [paymaster.address, 0, withdrawTokens],
          );
          ...
          const transferEvents = await paymaster.queryFilter(
            paymaster.filters.Transfer(),
            rcpt.blockHash,
          );
          const [log1, log2] = await entryPoint.queryFilter(
    ...
    describe("withdraw", () => {
      ...
      it("should be able to withdraw after unstake delay", async () => {
        ...
        expect(await ethers.provider.getBalance(withdrawAddress)).to.eql(
          amount,
        );
        expect(
          await entryPoint
