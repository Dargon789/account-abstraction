...
context("Token Paymaster", function () {
  ...
  before(async () => {
    ...
    await globalSigner.sendTransaction({
      to: ethersSigner.getAddress(),
      value: parseEther("10"),
    });
    ...
    const tokenInit = await new TestERC20__factory(
      ethersSigner,
    ).getDeployTransaction(6);
    ...
    const wethInit = await new TestWrappedNativeToken__factory(
      ethersSigner,
    ).getDeployTransaction();
    const wethAddress = await create2Factory.deploy(wethInit, 0);
    const testUniswapInit = await new TestUniswap__factory(
      ethersSigner,
    ...
    const nativeAssetOracleInit = await new TestOracle2__factory(
      ethersSigner,
    ).getDeployTransaction(initialPriceEther, 8);
    const nativeAssetOracleAddress = await create2Factory.deploy(
      nativeAssetOracleInit,
      0,
      10_000_000,
    );
    const tokenOracleInit = await new TestOracle2__factory(
      ethersSigner,
    ).getDeployTransaction(initialPriceToken, 8);
    const tokenOracleAddress = await create2Factory.deploy(
      tokenOracleInit,
      0,
      10_000_000,
    );
